...
class DisquaireTour:
    ...
    def welcome(self):
        ...
        if not Confirm.ask("Are you ready to begin your journey through the musical cosmos?"):
    ...
    def chapter_3_graph_revelation(self):
        ...
        if Confirm.ask(f"Launch the graph explorer on port {self.graph_port}?"):
...
[bold yellow]The tour never ends. The discoveries never stop.[/bold yellow]"""),
    ...
    def run_tour(self):
        ...
        try:
            ...
            if Confirm.ask("\\nContinue to the visual revelation?"):
                self.chapter_2_first_search()
            
            if Confirm.ask("\\nReady to see your music as a living constellation?"):
                self.chapter_3_graph_revelation()
            
            if Confirm.ask("\\nShall we unite exploration with musical experience?"):
                self.chapter_4_web_flow()
            
            if Confirm.ask("\\nAre you prepared for the deepest musical mystery?"):
